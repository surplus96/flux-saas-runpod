...
# ---------------------------------------------------------------------------
# Workflow configuration (Basic workflow)
# ---------------------------------------------------------------------------
...
WORKFLOW_PATH = Path(__file__).with_name("CTY_FLUX_KONTEXT_V2.json")
...
def update_mask_node(nodes, label: str):
    mask_node = get_node(nodes, MASK_NODE_ID)
    widgets = mask_node.setdefault("widgets_values", [])
    # Index 9 holds the textual description (default "Girl")
    while len(widgets) <= 9:
        widgets.append(None)
    widgets[9] = label
...
def validate_input(job_input):
    ...
    # Parameter-based path (basic workflow)
    try:
        workflow = build_basic_workflow(job_input)
